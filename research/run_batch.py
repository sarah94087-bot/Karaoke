...
for fname in files:
    ...
    print(
        f"  status={rows[-1]['status']} duration={rows[-1]['duration_sec']}s processing={rows[-1]['processing_sec']}s"
    )
    if not ok:
...
with open("output/report_timing.csv", "w", newline="", encoding="utf-8-sig") as f:
    writer = csv.DictWriter(
        f, fieldnames=["file", "status", "duration_sec", "processing_sec", "realtime_factor"]
    )
    writer.writeheader()
